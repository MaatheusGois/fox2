...
class Overlay: SKScene {
    ...
    init(size: CGSize, controller: GameController) {
        ...
        // The Gems
        for i in 0 ..< Constants.gemsCount {
            let gemNode = SKSpriteNode(imageNamed: "collectableBIG_empty.png")
            gemNode.position = CGPoint(x: 125 + i * 50, y: -50)
            gemNode.xScale = 0.25
            gemNode.yScale = 0.25
        ...
        collectedKeySprite.position = CGPoint(x: 125 + CGFloat(Constants.gemsCount) * 60, y: -50)
    ...
    func layout2DOverlay() {
        ...
        guard let congratulationsGroupNode = congratulationsGroupNode else { return }
